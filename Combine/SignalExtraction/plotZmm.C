...
void plotZmm(const TString  outputDir,   // output directory
  ...
  cout << yield << ", " << hData->Integral() << ", " << hData->Integral()-yield << endl;
  // Initial values for yields in Combine

  cout << "Starting values for Zmm yields: " << endl;
  cout << "   sig: " << hZmm->Integral() << endl;
  cout << "   EWK: " << hEWK->Integral() << endl;

  // Save mass distributions for Combine

  TFile* combineFile = new TFile("Zmm_massDists.root","RECREATE");
  hData->Write();
  hZmm->Write();
  hEWK->Write();
  combineFile->Close();
