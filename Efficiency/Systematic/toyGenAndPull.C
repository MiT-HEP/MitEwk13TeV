...
// std::cout << "cccccccccccc" << std::endl;
  ...
  TH1D *h = new TH1D(outHistName,outHistName,100,-10.0,10.0);
  ...
  for(int i =0; i < nPsExp; ++i){
      ...
      // dataPass = modelPass->generate(RooArgSet(m),NsigPass.getVal()+NbkgPass.getVal());
      // // dataFail = modelFail->generateBinned(RooArgSet(m),NsigFail.getVal()+NbkgFail.getVal());
      // dataFail = modelFail->generate(RooArgSet(m),NsigFail.getVal()+NbkgFail.getVal());
      ...
      std::cout <<"EFFICIENCY: "<<eff->getVal()<<" "<<fabs(eff->getErrorLo()) <<std::endl;//<< " pull " << (eff->getVal()-effMaster)/eff->getErrorLo()<< endl;
