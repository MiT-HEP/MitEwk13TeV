...
void fitWe_test_parfit(const TString  outputDir,   // output directory
...
//     recoilType = "Puppi";
  ...
  ///afs/cern.ch/work/x/xniu/public/WZXSection/wz-efficiency/";
//   const TString baseDir = "/afs/cern.ch/work/x/xniu/public/WZXSection/wz-efficiency/TighterIso/";
...
// 
//   f_hlt_data_pos = TFile::Open(baseDir1+"EleHLTEff/Nominal/EleTriggerTF1_Data_Positive.root");
//   f_hlt_mc_pos   = TFile::Open(baseDir1+"EleHLTEff/Nominal/EleTriggerTF1_MC_Positive.root");
//   f_hlt_data_neg = TFile::Open(baseDir1+"EleHLTEff/Nominal/EleTriggerTF1_Data_Negative.root");
//   f_hlt_mc_neg   = TFile::Open(baseDir1+"EleHLTEff/Nominal/EleTriggerTF1_MC_Negative.root");
//   f_hlt_mc_neg_b0= TFile::Open(baseDir1+"EleHLTEff/Erf/EleTriggerTF1_MC_Negative.root");
...
//   }
  ...
  // ======================= Recoil Corrections ================================
//   const TString directory("/afs/cern.ch/user/d/dalfonso/public/WZ/MAY3");
...
// // // // // 
  ...
  // Arrays to hold pointers to 1-dimensional histograms
  ...
  // Not sure I need these, but leave them for now
  ...
  TH1D *hElectronMETdPhi = new TH1D("hElectronMETdPhi","",50,0,TMath::Pi()); hElectronMETdPhi->Sumw2();
  
  TH2D *h2dIsoBinsData = new TH2D("h2dIsoBinsData","h2dIsoBinsData",nIsoBins,vIsoBins,NBINS,0,METMAX);
  TH2D *h2dIsoBinsDatap = new TH2D("h2dIsoBinsDatap","h2dIsoBinsDatap",nIsoBins,vIsoBins,NBINS,0,METMAX);
  TH2D *h2dIsoBinsDatam = new TH2D("h2dIsoBinsDatam","h2dIsoBinsDatam",nIsoBins,vIsoBins,NBINS,0,METMAX);
  
  TH2D *h2dIsoBinsSig = new TH2D("h2dIsoBinsSig","h2dIsoBinsSig",nIsoBins,vIsoBins,NBINS,0,METMAX);
  TH2D *h2dIsoBinsSigp = new TH2D("h2dIsoBinsSigp","h2dIsoBinsSigp",nIsoBins,vIsoBins,NBINS,0,METMAX);
  TH2D *h2dIsoBinsSigm = new TH2D("h2dIsoBinsSigm","h2dIsoBinsSigm",nIsoBins,vIsoBins,NBINS,0,METMAX);
  
  TH2D *h2dIsoBinsEwk = new TH2D("h2dIsoBinsEwk","h2dIsoBinsEwk",nIsoBins,vIsoBins,NBINS,0,METMAX);
  TH2D *h2dIsoBinsEwkp = new TH2D("h2dIsoBinsEwkp","h2dIsoBinsEwkp",nIsoBins,vIsoBins,NBINS,0,METMAX);
  TH2D *h2dIsoBinsEwkm = new TH2D("h2dIsoBinsEwkm","h2dIsoBinsEwkm",nIsoBins,vIsoBins,NBINS,0,METMAX);
...
//       if(fabs((lep_raw->Pt() - lep->Pt())/lep->Pt())>0.2) continue;
...
//       if(met < 20) continue;
     ...
     // ======================================
//      std::cout << "lep Eta " << lep->Eta() << "  iso " << pfCombIso << "  lep Pt " << lep->Pt() << "  lepPt*0.076="<< 0.0766*(lep->Pt())<< std::endl;
...
//      if((typev[ifile]==eData||typev[ifile]==eWenu||typev[ifile]==eEWK||typev[ifile]==eBKG||typev[ifile]==eQCD) && (( fabs(lep->Eta()) < 1.442 &&  pfCombIso >= 0.0766*(lep->Pt())) || (fabs(lep->Eta()) > 1.566 &&  pfCombIso >= 0.0678*(lep->Pt())))  ) continue;
    ...
    // // Check the isolation for the background region and fill the appropriate isolation bin
    // if(pfCombIso > 0.25 && pfCombIso < 0.35){
        // switch(typev[ifile]){
            // case eAntiData:
            ...
            // case eAntiEWK:
               // std::cout << "fill the antiEWK" << std::endl;
        // }
    // } else if(pfCombIso > 0.35 && pfCombIso < 0.45) {
    // } else if(pfCombIso > 0.45 && pfCombIso < 0.55) {
    // } else if(pfCombIso > 0.55 && pfCombIso < 0.65) {
    // } else if(pfCombIso > 0.65 && pfCombIso < 0.75) {
    // }
...
//           }
          ...
          {
//             std::cout << "Filling ZXX and WX info: Wp" << std::endl;
...
/*          else if(doPF) recoilCorrPFm->CorrectInvCdf(corrMet,corrMetPhi,genVPt,genVPhi,lep_raw->Pt(),lep->Phi(),pU1,pU2,0);
          ...
          {
//             std::cout << "Filling ZXX and WX info: Wm" << std::endl;
            hEWKMet->Fill(corrMetWithLepton,weight);
...
//             Double_t corrMetWithLepton = (vMetCorr + vLepRaw - vLepCor).Mod();
			...
			// sig2dVarBins->Fill(corrMetWithLepton, pfCombIso, weight2);
			h2dIsoBinsSigp->Fill(pfCombIso/(lep->Pt()),corrMetWithLepton,weight2);
            for(int it=0; it < nIsoBins; ++it){
                // check the isolation value for this event and fill appropriate histogram
...
//             Double_t corrMetWithLepton = (vMetCorr + vLepRaw - vLepCor).Mod();
			...
			h2dIsoBinsSigm->Fill(corrMetWithLepton, pfCombIso/(lep->Pt()),weight2);
            for(int it=0; it < nIsoBins; ++it){
                // check the isolation value for this event and fill appropriate histogram
                if(pfCombIso/(lep->Pt()) > vIsoBins[it] && pfCombIso/(lep->Pt()) < vIsoBins[it+1]) {
        ...
        if(typev[ifile]==eEWK) {
          ...
          if(q>0) {
//             std::cout << "Filling other EWK info: Wp" << std::endl;
            hEWKMetp->Fill(corrMetWithLepton,weight); 
            hEWKMetp_PileupUp->Fill(corrMetWithLepton,weightUp); 
            hEWKMetp_PileupDown->Fill(corrMetWithLepton,weightDown); 
			h2dIsoBinsEwkp->Fill(pfCombIso/(lep->Pt()),corrMetWithLepton,weight);
...
//           Double_t corrMetWithLepton = (vMetCorr + vLepRaw - vLepCor).Mod();
          ...
          if(q>0) { 
			...
			// ewk2dVarBins->Fill(corrMetWithLepton, pfCombIso, weight2);
			h2dIsoBinsEwkp->Fill(pfCombIso/(lep->Pt()),corrMetWithLepton,weight2);
            for(int it=0; it < nIsoBins; ++it){
                // check the isolation value for this event and fill appropriate histogram
                if(pfCombIso/(lep->Pt()) > vIsoBins[it] && pfCombIso/(lep->Pt()) < vIsoBins[it+1]) {
			...
			h2dIsoBinsEwkm->Fill(pfCombIso/(lep->Pt()),corrMetWithLepton,weight2);
            for(int it=0; it < nIsoBins; ++it){
                // check the isolation value for this event and fill appropriate histogram
                if(pfCombIso/(lep->Pt()) > vIsoBins[it] && pfCombIso/(lep->Pt()) < vIsoBins[it+1]) {
                    hAntiEWKMetmIsoBins[it]->Fill(corrMetWithLepton, weight2);
        ...
        if(typev[ifile]==eQCD) {
          if(lep->Pt()        < PT_CUT)  continue;
          TVector2 vMetCorr((met)*cos(metPhi),(met)*sin(metPhi));
          TVector2 corrMetWithLepton = -1*(vMetCorr + vLepCor);
          hQCDMet->Fill(corrMetWithLepton.Mod(),weight);
          hElectronMETdPhi->Fill(toolbox::deltaPhi(corrMetWithLepton.Phi(), lep->Phi()),weight);
//           hQCDMet->Fill(corrMetWithLepton,weight);
          if(q>0) {
//             hQCDMetp->Fill(corrMetWithLepton,weight); 
            hQCDMetp->Fill(met,weight);
          }
          else { 
//             hQCDMetm->Fill(corrMetWithLepton,weight); 
            hQCDMetm->Fill(met,weight);
          }
        }
        if(typev[ifile]==eAntiQCD) {
          if(lep->Pt()        < PT_CUT)  continue;
          TVector2 vMetCorr((met)*cos(metPhi),(met)*sin(metPhi));
          Double_t corrMetWithLepton = met;//(vMetCorr + vLepRaw - vLepCor).Mod();
...
//   hWenuMetm_ScaleDown->Add(corrM,hWenuMetm,1);
  ...
  RooRealVar nAntiSig("nAntiSig","nAntiSig",hAntiDataMet->Integral()*0.05,0,hAntiDataMet->Integral());
...
//   cewkm.setConstant(kTRUE);
  ...
  RooRealVar nAntiSigm("nAntiSigm","nAntiSigm",hAntiDataMetm->Integral()*0.05,0,hAntiDataMetm->Integral());
//   nAntiSigm.setVal(0);
//   nAntiSigm.setConstant(kTRUE);
  RooRealVar nAntiQCDm("nAntiQCDm","nAntiQCDm",0.90*(hAntiDataMetm->Integral()),0,hAntiDataMetm->Integral());
  ...
  // set up the variables for the 2-dimentional fits
  RooRealVar nSigp2d("nSigp2d","nSigp2d",0.85*h2dIsoBinsDatap->Integral(),0,h2dIsoBinsDatap->Integral());
  //RooRealVar nQCDp("nQCDp","nQCDp",0.3*(hDataMetp->Integral()),0,hDataMetp->Integral());
  RooRealVar nQCDp2d("nQCDp2d","nQCDp2d",0.05*h2dIsoBinsDatap->Integral(),0,h2dIsoBinsDatap->Integral());
  RooRealVar cewkp2d("cewkp2d","cewkp2d",0.1,0,1.0);
  cewkp2d.setVal(h2dIsoBinsEwkp->Integral()/h2dIsoBinsSigp->Integral());
  cewkp2d.setConstant(kTRUE);
  RooFormulaVar nEWKp2d("nEWKp2d","nEWKp2d","cewkp2d*nSigp2d",RooArgList(nSigp2d,cewkp2d));
  
  RooRealVar nSigm2d("nSigm2d","nSigm2d",0.85*h2dIsoBinsDatam->Integral(),0,h2dIsoBinsDatam->Integral());
  //RooRealVar nQCDp("nQCDp","nQCDp",0.3*(hDataMetp->Integral()),0,hDataMetp->Integral());
  RooRealVar nQCDm2d("nQCDm2d","nQCDm2d",0.05*h2dIsoBinsDatam->Integral(),0,h2dIsoBinsDatam->Integral());
  RooRealVar cewkm2d("cewkm2d","cewkm2d",0.1,0,1.0);
  cewkm2d.setVal(h2dIsoBinsEwkm->Integral()/h2dIsoBinsSigm->Integral());
  ...
  // make a variable for the isolation bins:
  RooRealVar iso("iso","iso",0.25,0.65);
  iso.setBins(4);
  ...
  // Anti-EWK+top PDFs
  RooDataHist aewkMet ("aewkMET", "aewkMET", RooArgSet(pfmet,iso),hAntiEWKMet);  RooHistPdf apdfEWK ("aewk", "aewk", pfmet,aewkMet, 1);
  ...
  // The 2-d histograms / pdfs
  // RooDataHist wenu2dMet ("wenuMET", "wenuMET", RooArgSet(pfmet),hWenuMet);  RooHistPdf pdfWe ("we", "we", pfmet,wenuMet, 1);
  RooDataHist wenuMetp2d("wenuMetp2d","wenuMetp2d",RooArgSet(iso,pfmet),h2dIsoBinsSigp); RooHistPdf pdfWep2d("wep2d","wep2d",RooArgSet(iso,pfmet),wenuMetp2d,1);
  RooDataHist wenuMetm2d("wenuMetm2d","wenuMetm2d",RooArgSet(iso,pfmet),h2dIsoBinsSigm); RooHistPdf pdfWem2d("wem2d","wem2d",RooArgSet(iso,pfmet),wenuMetm2d,1); 
  
  // EWK+top PDFs
  // RooDataHist ewk2dMet ("ewkMET", "ewkMET", RooArgSet(pfmet),hEWKMet);  RooHistPdf pdfEWK ("ewk", "ewk", pfmet,ewkMet, 1);
  RooDataHist ewkMetp2d("ewkMetp2d","ewkMetp2d",RooArgSet(iso,pfmet),h2dIsoBinsEwkp); RooHistPdf pdfEWKp2d("ewkp2d","ewkp2d",RooArgSet(iso,pfmet),ewkMetp2d,1); 
  RooDataHist ewkMetm2d("ewkMetm2d","ewkMetm2d",RooArgSet(iso,pfmet),h2dIsoBinsEwkm); RooHistPdf pdfEWKm2d("ewkm2d","ewkm2d",RooArgSet(iso,pfmet),ewkMetm2d,1); 
...
//   // Anti-selection PDFs
  ...
  RooAddPdf pdfMetp2d("pdfMetp2d","pdfMetp2d",RooArgList(/*pdfWep2d,pdfEWKp2d,*/*(qcdp2d.model)),RooArgList(/*nSigp2d,nEWKp2d,*/nQCDp2d));
  RooAddPdf pdfMetm2d("pdfMetm2d","pdfMetm2d",RooArgList(pdfWem2d,pdfEWKm2d,*(qcdm2d.model)),RooArgList(nSigm2d,nEWKm2d,nQCDm2d)); 
...
//   RooAddPdf apdfMetm("apdfMetm","apdfMetm",RooArgList(apdfWem,apdfEWKm,apdfQCDm),RooArgList(nAntiSigm,nAntiEWKm,nAntiQCDm));
  ...
  RooDataHist dataMetp2D("dataMetp2D","dataMetp2D",RooArgSet(iso,pfmet),h2dIsoBinsDatap);
  RooDataHist dataMetm2D("dataMetm2D","dataMetp2D",RooArgSet(iso,pfmet),h2dIsoBinsDatam);
...
// return;
 ...
 std::cout << "after workspace" << std::endl;

  RooFitResult *fitResp2D = pdfMetp2d.fitTo(dataMetp2D,Extended(),Save(kTRUE));
  RooFitResult *fitResm2D = pdfMetm2d.fitTo(dataMetm2D,Extended(),Save(kTRUE));
  // RooFitResult *fitResp2D = qcdp2d.model->fitTo(dataMetp2D,Extended(),Save(kTRUE));
...
//   RooFitResult *fitResAntip = apdfMetp.fitTo(antiMetp,Extended(),Minos(kTRUE),Save(kTRUE));
  ...
  // RooFitResult *fitResm = pdfMetm.fitTo(dataMetm,Extended(),ExternalConstraints(RooArgSet(constm/*,fconsta1m,fconsta2m,fconsta3m*/)),Minos(kTRUE),Save(kTRUE));
...
//   RooFitResult *fitResAntim = apdfMetm.fitTo(antiMetm,Extended(),Minos(kTRUE),Save(kTRUE));
  ...
  for(int it = 0; it < h2dIsoBinsDatap->GetNbinsX();++it){
...
//     RooPlot *lFrame1 = lRXVar.frame(Title(TString("Projection Range: "+ int2string( i0*((int)(fZPtMax/lNBins))) + "  to  " +int2string(i0*((int)(fZPtMax/lNBins))+((int)(fZPtMax/lNBins))) + " [GeV]" )));
  ...
  reduce->plotOn(lFrame1,MarkerStyle(kFullCircle),MarkerSize(0.9),DrawOption("ZP"));
  pdfMetp2d.plotOn(lFrame1,FillColor(fillcolorW),DrawOption("F"));
  pdfMetp2d.plotOn(lFrame1,LineColor(linecolorW));
  pdfMetp2d.plotOn(lFrame1,Components(RooArgSet(pdfEWKp2d,*(qcdp2d.model))),FillColor(fillcolorEWK),DrawOption("F"));
  pdfMetp2d.plotOn(lFrame1,Components(RooArgSet(pdfEWKp2d,*(qcdp2d.model))),LineColor(linecolorEWK));
  pdfMetp2d.plotOn(lFrame1,Components(RooArgSet(*(qcdp2d.model))),FillColor(fillcolorQCD),DrawOption("F"));
  pdfMetp2d.plotOn(lFrame1,Components(RooArgSet(*(qcdp2d.model))),LineColor(linecolorQCD));
  pdfMetp2d.plotOn(lFrame1,Components(RooArgSet(pdfWep2d)),LineColor(linecolorW),LineStyle(2));
...
//     RooPlot *lFrame1 = lRXVar.frame(Title(TString("Projection Range: "+ int2string( i0*((int)(fZPtMax/lNBins))) + "  to  " +int2string(i0*((int)(fZPtMax/lNBins))+((int)(fZPtMax/lNBins))) + " [GeV]" )));
  // RooPlot *lFrame1 = pfmet.frame(Title(TString("Projection Range: "+ to_string(vIsoBins[i]) + "  to  " + to_string(vIsoBins[i+1]) + " iso" )));
  // reduce->plotOn(lFrame1);
  reduce2->plotOn(lFrame1,MarkerStyle(kFullCircle),MarkerSize(0.9),DrawOption("ZP"));
  pdfMetm2d.plotOn(lFrame1,FillColor(fillcolorW),DrawOption("F"));
  pdfMetm2d.plotOn(lFrame1,LineColor(linecolorW));
  pdfMetm2d.plotOn(lFrame1,Components(RooArgSet(pdfEWKm2d,*(qcdm2d.model))),FillColor(fillcolorEWK),DrawOption("F"));
  pdfMetm2d.plotOn(lFrame1,Components(RooArgSet(pdfEWKm2d,*(qcdm2d.model))),LineColor(linecolorEWK));
  pdfMetm2d.plotOn(lFrame1,Components(RooArgSet(*(qcdm2d.model))),FillColor(fillcolorQCD),DrawOption("F"));
  pdfMetm2d.plotOn(lFrame1,Components(RooArgSet(*(qcdm2d.model))),LineColor(linecolorQCD));
  pdfMetm2d.plotOn(lFrame1,Components(RooArgSet(pdfWem2d)),LineColor(linecolorW),LineStyle(2));
  ...
  sprintf(txtfname1,"%s/fitresWe2d.txt",CPlot::sOutDir.Data());
  ...
  // txtfile << " *** Yields *** " << endl;
  txtfile1 << "Selected: " << h2dIsoBinsDatap->Integral() << endl;
  ...
  fitResp2D->printStream(txtfile1,RooPrintable::kValue,RooPrintable::kVerbose);
  txtfile1 << endl;
  
  fitResm2D->printStream(txtfile1,RooPrintable::kValue,RooPrintable::kVerbose);
  txtfile1 << endl;
  txtfile1.close();
  ...
  std::cout << "did diff 6" << std::endl;

  std::cout << "finished diffs for 1-d" << std::endl;
  //--------------------------------------------------------------------------------------------------------------
  // Make plots 
  //==============================================================================================================  
  ...
  hDummyQCD->SetFillStyle(1001);
   ...
   std::cout << "finished setting up for plots" << std::endl;
  //
  // W MET plot
  //
...
//   plotMet.Draw(c,kTRUE,format,1);
  ...
  c->Clear();
  hElectronMETdPhi->GetYaxis()->SetRangeUser(0,hElectronMETdPhi->GetMaximum()*1.05);
  hElectronMETdPhi->SetTitle("dPhi between electron & MET");
  hElectronMETdPhi->GetXaxis()->SetTitle("dPhi");
  hElectronMETdPhi->Draw();
  c->SaveAs(outputDir+"/hElectronMETdPhi.png");
  
  std::cout << "finished plotting 1-d" << std::endl;
...
//   ksprobpe = hDataMetp->KolmogorovTest(hPdfMetp,"DX");  
  ...
  fitResp2D->printStream(txtfile,RooPrintable::kValue,RooPrintable::kVerbose);
  txtfile << endl;
  // fitResAntip->printStream(txtfile,RooPrintable::kValue,RooPrintable::kVerbose);
  txtfile << endl;
  printCorrelations(txtfile, fitResp2D);
  txtfile << endl;
  printChi2AndKSResults(txtfile, chi2prob, chi2ndf, ksprob, ksprobpe);
  txtfile.close();
