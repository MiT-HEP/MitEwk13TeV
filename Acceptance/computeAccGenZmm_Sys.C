...
void computeAccGenZmm_Sys(const TString conf,             // input file
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
    ...
    for(UInt_t ientry=0; ientry<(uint)(0.5*eventTree->GetEntries()); ientry++) {
      ...
      // TLorentzVector *glep1=*lep1;
	  // TLorentzVector *glep2=*lep2;
      ...
      // TLorentzVector *gvec = glep1+glep2;
      // if(gvec->M() > MASS_LOW && gvec->M < MASS_HIGH &&glep1->Pt() > PT_CUT && glep2->PT() > PT_CUT && fabs(glep1->Eta()) < ETA_CUT && fabs(glep2->Eta()) < ETA_CUT){
