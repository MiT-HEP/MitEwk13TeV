...
void computeAccGenWe_Sys(const TString conf,       // input file
  ...
  gBenchmark->Start("computeAccGenWe_Sys");
  ...
  while(getline(ifs,line)) {
    ...
    colorv.push_back(color); // since this one is unused, i'm going to store the cross-section in it so i can use multiple inputs
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {  
    ...
    // for(int npdf=0; npdf<NPDF; npdf++){
        // accv_PDF.push_back(xsec*nSelv_PDF[npdf]/nEvtsv_PDF[npdf]);     
        ...
        // accv_uncPDF+=sqrt((accv_PDF[npdf]-accv[ifile])*(accv_PDF[npdf]-accv[ifile]))/NPDF;
        // accv_uncPDF_num+=sqrt((nSelv_PDF[npdf]-nSelv[ifile])*(nSelv_PDF[npdf]-nSelv[ifile]))/(NPDF*nSelv[ifile]);
        // accv_uncPDF_dnm+=sqrt((nEvtsv_PDF[npdf]-nEvtsv[ifile])*(nEvtsv_PDF[npdf]-nEvtsv[ifile]))/(NPDF*nEvtsv[ifile]);
    // }
    ...
    // for(int nqcd=0; nqcd<NQCD; nqcd++){
        ...
        // std::cout << "nselvqcd " << nSelv_QCD[nqcd] << "  nevtsvqcd " << nEvtsv_QCD[nqcd] << " ratio " << nSelv_QCD[nqcd]/nEvtsv_QCD[nqcd] << std::endl;
        // std::cout << "accv " << accv[ifile] << "  accvqcd " << accv_QCD[nqcd] << std::endl;
        // std::cout << "diff " << accv_QCD[nqcd]-accv[ifile] << "  pct diff " << 100*(accv_QCD[nqcd]-accv[ifile])/accv[ifile] << std::endl;
        // accErrv_QCD.push_back(sqrt(accv_QCD[nqcd]*(1.-accv_QCD[nqcd])/nEvtsv_QCD[nqcd]));
        // // a = abs((acc_i-accTot)/accTot)
        // if(fabs(accv_QCD[nqcd]-accv[ifile]) > accv_uncQCD) accv_uncQCD = fabs(accv_QCD[nqcd]-accv[ifile])/(accv[ifile]);
        // if(fabs(nSelv_QCD[nqcd]-nSelv[ifile]) > accv_uncQCD_num) accv_uncQCD_num = fabs(nSelv_QCD[nqcd]-nSelv[ifile])/nSelv[ifile];
        // if(fabs(nEvtsv_QCD[nqcd]-nEvtsv[ifile]) > accv_uncQCD_dnm) accv_uncQCD_dnm = fabs(nEvtsv_QCD[nqcd]-nEvtsv[ifile])/nEvtsv[ifile];
    // }
