...
//	  std::cout<<vTag.Pt()<<", "<<vTag.Eta()<<", "<<vTag.Phi()<<", "<<vTag.E()<<std::endl;
	  ...
	  if(doScaleCorr && (tag->r9 < 1.)){
            // set up variable and apply scale and smear correction
            // only apply correction to GsfElectron, not SC, since we never use tag SC info.
            float tagSmear = 0.;
            float tagScale = 1.;
...
//	  pfCombIso1 = tag->chHadIso + TMath::Max(tag->neuHadIso + tag->gammaIso - (info->rhoIso)*getEffAreaEl(tag->scEta), 0.);
	...
	for(Int_t j=0; j<scArr->GetEntriesFast(); j++) {
	  ...
	  if(scProbe->scID == tagscID) continue;
          ...
          vProbeSC.SetPtEtaPhiM(scProbe->scEt, scProbe->scEta, scProbe->scPhi, ELE_MASS);

	  float probeError = 0.;
	  float probeSCError = 0.;
          if(doScaleCorr && (scProbe->r9 < 1.)){
          // set up variable and apply scale and smear correction
          // apply corrections both for GsfSel and SC, since we need to access probe SC info later when doing GSfSelEff
            float probeSmear = 0.;
            float probeScale = 1.;

            float probeSCSmear = 0.;
            float probeSCScale = 1.;

            float probeAbsEta   = fabs(vProbe.Eta());
            float probeEt       = vProbe.E() / cosh(probeAbsEta);
            bool  probeisBarrel = probeAbsEta < 1.4442;

            float probeSCAbsEta   = fabs(vProbeSC.Eta());
            float probeSCEt       = vProbeSC.E() / cosh(probeSCAbsEta);
            bool  probeSCisBarrel = probeSCAbsEta < 1.4442;
            ...
            if(snamev[isam].CompareTo("data",TString::kIgnoreCase)==0){//Data
              ...
              probeSCScale = eleCorr.ScaleCorrection(info->runNum, probeSCisBarrel, scProbe->r9, probeSCAbsEta, probeSCEt);
              probeSCError = eleCorr.ScaleCorrectionUncertainty(info->runNum, probeSCisBarrel, scProbe->r9, probeSCAbsEta, probeSCEt);

              (vProbe) *= probeScale;
	      (vProbeSC) *= probeSCScale;
              ...
              probeSCSmear = eleCorr.getSmearingSigma(info->runNum, probeSCisBarrel, scProbe->r9, probeSCAbsEta, probeSCEt, 0., 0.);
              float probeSCSmearE = probeSCSmear + std::hypot(eleCorr.getSmearingSigma(info->runNum, probeSCisBarrel, scProbe->r9, probeSCAbsEta, probeSCEt, 1., 0.) - probeSCSmear,  eleCorr.getSmearingSigma(info->runNum, probeSCisBarrel, scProbe->r9, probeSCAbsEta, probeSCEt, 0., 1.) - probeSCSmear);
              double probeSCRamdom = gRandom->Gaus(0,1);
              probeSCError = vProbeSC.E() * (1.0 + probeSCSmearE * probeSCRamdom);
              probeSCSmear = 1. + probeSCSmear * probeSCRamdom;

              (vProbe) *= probeSmear;
	      (vProbeSC) *= probeSCSmear;
            }
          }
...
//	  if(fabs(scProbe->eta)  > ETA_CUT) continue;  // Supercluster |eta| cuts
	  ...
	  TLorentzVector vEleProbe(0,0,0,0);
	  if(eleProbe){
...
//					    (info->rhoIso)*getEffAreaEl(eleProbe->scEta), 0.) :  -1;
	  ...
	  lep2error  = (eleProbe) ? eleProbeError : probeError;
