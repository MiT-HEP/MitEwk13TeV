...
void plotWeMet(const TString  conf,      // input file
               const TString  inputDir,  // input directory
	       const TString  outputDir, // output directory
	       const Double_t lumi       // integrated luminosity (/fb)
) {
  gBenchmark->Start("plotWeMet");
 ...
 for(UInt_t ich=0; ich<3; ich++) {
    for(UInt_t isam=0; isam<samplev.size(); isam++) {
      ...
      sprintf(hname,"hMet2_ch%i_%i",ich,isam);     hMet2v[ich].push_back(new TH1D(hname,"",50,0,200));       hMet2v[ich][isam]->Sumw2();
  ...
  for(UInt_t ich=0; ich<3; ich++) {    
    ...
    if(ich==eInc) sprintf(sigtext,"W#rightarrowe#nu");
    else if(ich==ePos) sprintf(sigtext,"W^{+}#rightarrowe^{+}#nu");
    else if(ich==eNeg) sprintf(sigtext,"W^{-}#rightarrowe^{-}#nu");
...
//    plotMet.AddHist1D(hMetv[ich][0],samplev[0]->label,"E"); // No data to plot
    ...
    for(UInt_t isam=1; isam<samplev.size(); isam++) {
      ...
      else if(samplev[isam]->label=="W#rightarrowe#nu") nsig_pf += hMetv[ich][isam]->Integral();
    ...
    cout << "W->enu pf met --- signal yield is " << nsig_pf << endl;
...
//    plotMet_pf.SetLegend(0.68,0.57,0.84,0.70);
    plotMet_pf.SetLegend(0.7,0.45,0.9,0.75); 
...
//    plotMet2_pf.SetLegend(0.68,0.57,0.84,0.70);
    plotMet2_pf.SetLegend(0.7,0.45,0.9,0.75); 
...
//    plotMet.AddHist1D(hMetv[ich][0],samplev[0]->label,"E"); // No data to plot
    ...
    for(UInt_t isam=1; isam<samplev.size(); isam++) {
      ...
      else if(samplev[isam]->label=="W#rightarrowe#nu") nsig_trk += hTrkMetv[ich][isam]->Integral();
    ...
    cout << "W->enu track met --- signal yield is " << nsig_trk << endl;
...
//    plotMet_trk.SetLegend(0.68,0.57,0.84,0.70);
    plotMet_trk.SetLegend(0.7,0.45,0.9,0.75); 
...
//    plotMet2_trk.SetLegend(0.68,0.57,0.84,0.70);
    plotMet2_trk.SetLegend(0.7,0.45,0.9,0.75);
