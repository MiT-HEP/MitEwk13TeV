...
void fitRecoilZmm3(TString infilename="/data/blue/Bacon/Run2/wz_flat/Zmumu/ntuples/data_select.root",  // input ntuple
...
//     intree->SetBranchAddress("puWeight",     &puWeight); 
    ...
    for(Int_t ientry=0; ientry<intree->GetEntries(); ientry++) {
      ...
      if(isBkgv[ifile]) {
        hPFu1Bkgv[ipt]->Fill(u1,scale1fb*2318);//*puWeight*42);
        hPFu2Bkgv[ipt]->Fill(u2,scale1fb*2318);//*puWeight*42);
      
      } else {
        hPFu1v[ipt]->Fill(u1,scale1fb*2318);//*puWeight*42);
        hPFu2v[ipt]->Fill(u2,scale1fb*2318);//*puWeight*42);
      }
    }
...
void performFit(const vector<TH1D*> hv, const vector<TH1D*> hbkgv, const Double_t *ptbins, const Int_t nbins,
  ...
  for(Int_t ibin=0; ibin<nbins; ibin++) {
    ...
    RooRealVar sigma1(name.str().c_str(),name.str().c_str(),0.5*(hv[ibin]->GetRMS()),0,1.5*(hv[ibin]->GetRMS()));
    name.str(""); name << "sigma2_" << ibin;
    RooRealVar sigma2(name.str().c_str(),name.str().c_str(),1.0*(hv[ibin]->GetRMS()),0,2.0*(hv[ibin]->GetRMS()));
    name.str(""); name << "sigma3_" << ibin;
    RooRealVar sigma3(name.str().c_str(),name.str().c_str(),2*(hv[ibin]->GetRMS()),0,10*(hv[ibin]->GetRMS())); 
...
/*    // Works for Zmm Data no bkg-sub to get all U1 set
    ...
    if(ibin>0) {
      ...
      if(ibin == 15 || ibin == 22 || ibin == 35 || ibin == 10 || ibin == 28 || ibin == 52 || ibin == 17 || ibin == 31|| ibin ==36|| ibin ==46|| ibin ==47||ibin==54||ibin==57||ibin==29||ibin==21||ibin==53||ibin==56||ibin==58||ibin==38){
      mean1.setVal(hv[ibin]->GetMean());
      mean2.setVal(hv[ibin]->GetMean());
      mean3.setVal(hv[ibin]->GetMean());
    ...
    if( ibin == 18 || ibin ==34 || ibin ==35||ibin==49 || ibin==24||ibin==27||ibin==55){
      mean1.setVal(hv[ibin]->GetMean());
      mean2.setVal(hv[ibin]->GetMean());
      mean3.setVal(hv[ibin]->GetMean());
...
// //     wksp->import(*modelcdf,RooFit::RecycleConflictNodes(),RooFit::Silence());
//     
//     RooRealVar* myXd = (RooRealVar*) modelpdf.getVariables()->find("u");
//     std::cout << "done " << std::endl;
//     std::cout << "max = " << myXd->getMin() << "  " << myXd->getMax() << std::endl;
//     
//     wksp->Print();
