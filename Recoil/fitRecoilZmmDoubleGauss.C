...
void fitRecoilZmmDoubleGauss(TString infilename="/data/blue/Bacon/Run2/wz_flat/Zmumu/ntuples/data_select.root",  // input ntuple
  ...
  //Double_t ptbins[] = {0,5,10,20,30,40,50,60,80,100};// Zmm data binning
  Double_t ptbins[] = {0,40,100};
  ...
  TString formulaPFu1mean1("pol1");
  TString formulaPFu1mean2("pol1");
  
  TString formulaPFu2mean("pol1");
  TString formulaPFu2mean1("pol1");
  TString formulaPFu2mean2("pol1");
...
//   fnamev.push_back("/data/blue/Bacon/Run2/wz_flat_07_23/Zmumu/ntuples/zxx_select.root"); isBkgv.push_back(kTRUE);
  ...
  TFitResultPtr fitresPFu1mean1;   TF1 *fcnPFu1mean1 = new TF1("fcnPFu1mean1",formulaPFu1mean1,0,7000);
  TFitResultPtr fitresPFu1mean2;   TF1 *fcnPFu1mean2 = new TF1("fcnPFu1mean2",formulaPFu1mean2,0,7000);
  ...
  TFitResultPtr fitresPFu2mean1;   TF1 *fcnPFu2mean1 = new TF1("fcnPFu2mean1",formulaPFu2mean1,0,7000);
  TFitResultPtr fitresPFu2mean2;   TF1 *fcnPFu2mean2 = new TF1("fcnPFu2mean2",formulaPFu2mean2,0,7000);
...
// // // for the zmm data pf correcionts
//    // ---
//   fcnPFu1sigma1->SetParameter(0,0); fcnPFu1sigma1->SetParLimits(0,-5e-2,5e-2);
...
//     fcnPFu2sigma0->SetParameter(0,-1e-3);  fcnPFu2sigma0->SetParLimits(0,-1e-3,1e-3);
//     fcnPFu2sigma0->SetParameter(1,0.03);   fcnPFu2sigma0->SetParLimits(1,-1,2);
//    // ----
   
//    // for the zmm data mva corrections
//       fcnPFu1sigma1->SetParameter(0,2e-4); fcnPFu1sigma1->SetParLimits(0,-5e-2,0);
//    fcnPFu2sigma1->SetParameter(0,-5e-4);  fcnPFu2sigma1->SetParLimits(0,-5e-3,0);
//     fcnPFu2sigma0->SetParameter(0,-1e-3);  fcnPFu2sigma0->SetParLimits(0,-1e-3,1e-3);
//     fcnPFu2sigma0->SetParameter(1,0.03);   fcnPFu2sigma0->SetParLimits(1,-1,2);
  // -------------  
  // Zmm PF MC
      ...
      fcnPFu1sigma1->FixParameter(0,0);// fcnPFu1sigma1->SetParLimits(0,-1e-3,0);
...
// //   
   fcnPFu1sigma2->FixParameter(0,0); // fcnPFu1sigma2->SetParLimits(0,0,0);
...
// //   
     fcnPFu1sigma0->FixParameter(0,0);// fcnPFu1sigma0->SetParLimits(0,0,0);
...
// //   
  // fcnPFu1frac2->FixParameter(0,0); //fcnPFu1frac2->SetParLimits(0,0,0);
...
// //   
// //   
  fcnPFu2sigma1->FixParameter(0,0);  //fcnPFu2sigma1->SetParLimits(0,0,0);
...
// //   
    fcnPFu2sigma2->FixParameter(0,0); // fcnPFu2sigma2->SetParLimits(0,0,0);
  fcnPFu2sigma2->SetParameter(1,-0.05);   fcnPFu2sigma2->SetParLimits(1,0,1);
//    fcnPFu2sigma2->SetParameter(2,10);      fcnPFu2sigma2->SetParLimits(2,5,12);
// //   
   fcnPFu2sigma0->FixParameter(0,0); // fcnPFu2sigma0->SetParLimits(0,0,0);
...
//   fcnPFu2frac2->SetParameter(2,7);     fcnPFu2frac2->SetParLimits(2,5,9); 
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
    ...
    for(Int_t ientry=0; ientry<intree->GetEntries(); ientry++) {
      ...
      if(isBkgv[ifile]) {
        hPFu1Bkgv[ipt]->Fill(u1/*,scale1fb*puWeight*10000*/);
        hPFu2Bkgv[ipt]->Fill(u2/*,scale1fb*puWeight*10000*/);
      
      } else {
        hPFu1v[ipt]->Fill(u1/*,scale1fb*puWeight*10000*/);
        hPFu2v[ipt]->Fill(u2/*,scale1fb*puWeight*10000*/);
      }
    }
  ...
  TGraphErrors *grPFu1mean1=0;  Double_t pfu1Mean1[nbins],  pfu1Mean1Err[nbins];
  ...
  TGraphErrors *grPFu2mean1=0;  Double_t pfu2Mean1[nbins],  pfu2Mean1Err[nbins];
  ...
  performFit(hPFu1v, hPFu1Bkgv, ptbins, nbins, pfu1model, sigOnly,
             c, "pfu1", (metName+" u_{1} [GeV]").c_str(),
         pfu1Mean,   pfu1MeanErr,
	     pfu1Mean1,  pfu1Mean1Err,
  ...
  performFit(hPFu2v, hPFu2Bkgv, ptbins, nbins, pfu2model, sigOnly,
             c, "pfu2", (metName+" u_{2} [GeV]").c_str(),
         pfu2Mean,   pfu2MeanErr,
	     pfu2Mean1,  pfu2Mean1Err,
...
//  plotPFu1mean.AddGraph(errBand,"3",kAzure-9,kFullDotSmall);
  ...
  grPFu1mean1 = new TGraphErrors(nbins,xval,pfu1Mean1,xerr,pfu1Mean1Err);
  grPFu1mean1->SetName("grPFu1mean1");
  fitresPFu1mean1 = grPFu1mean1->Fit("fcnPFu1mean1","QMRN0FBSE");
  sprintf(chi2ndf,"#chi^{2}/ndf = %.2f",(fcnPFu1mean1->GetChisquare())/(fcnPFu1mean1->GetNDF()));
//  errBand->SetPoint(0,0.002*(xval[nbins-1]),fcnPFu1mean->Eval(0.002*(xval[nbins-1])));
//  errBand->SetPointError(0,0,dMean(fcnPFu1mean,0.002*(xval[nbins-1]),fitresPFu1mean));
//  for(Int_t i=1; i<=nbins; i++) {
//    errBand->SetPoint(i,xval[i-1],fcnPFu1mean->Eval(xval[i-1]));
...
//  errBand->SetPoint(nbins+1,1.2*(xval[nbins-1]),fcnPFu1mean->Eval(1.2*(xval[nbins-1])));
//  errBand->SetPointError(nbins+1,0,dMean(fcnPFu1mean,1.2*(xval[nbins-1]),fitresPFu1mean));  
  CPlot plotPFu1mean1("pfu1mean1","","p_{T}(ll) [GeV/c]","#mu_{1}(u_{1}) [GeV]");
  plotPFu1mean1.AddGraph(grPFu1mean1,"");
//  plotPFu1mean.AddGraph(errBand,"3",kAzure-9,kFullDotSmall);
  plotPFu1mean1.AddGraph(grPFu1mean1,"",kBlack,kOpenCircle);
  plotPFu1mean1.AddFcn(fcnPFu1mean1,kRed);
  plotPFu1mean1.AddTextBox(chi2ndf,0.65,0.87,0.95,0.82,0,kBlack,-1);
  sprintf(fitparam,"p_{0} = %.3f #pm %.3f",fcnPFu1mean1->GetParameter(0),fcnPFu1mean1->GetParError(0)); plotPFu1mean1.AddTextBox(fitparam,0.65,0.80,0.95,0.75,0,kBlack,-1);
  sprintf(fitparam,"p_{1} = %.3f #pm %.3f",fcnPFu1mean1->GetParameter(1),fcnPFu1mean1->GetParError(1)); plotPFu1mean1.AddTextBox(fitparam,0.65,0.75,0.95,0.70,0,kBlack,-1);
  plotPFu1mean1.Draw(c,kTRUE,"png");
...
//  errBand->SetPoint(nbins+1,1.2*(xval[nbins-1]),fcnPFu1mean->Eval(1.2*(xval[nbins-1])));
//  errBand->SetPointError(nbins+1,0,dMean(fcnPFu1mean,1.2*(xval[nbins-1]),fitresPFu1mean));  
  CPlot plotPFu1mean2("pfu1mean2","","p_{T}(ll) [GeV/c]","#mu_{2}(u_{1}) [GeV]");
...
//  errBand->SetPointError(nbins+1,0,dMean(fcnPFu2mean,1.2*(xval[nbins-1]),fitresPFu2mean));    
  CPlot plotPFu2mean("pfu2mean","","p_{T}(ll) [GeV/c]","#mu_{1}(u_{2}) [GeV]");
...
//  plotPFu2mean.AddGraph(errBand,"3",kAzure-9,kFullDotSmall);
  ...
  grPFu2mean1 = new TGraphErrors(nbins,xval,pfu2Mean1,xerr,pfu2Mean1Err);
  grPFu2mean1->SetName("grPFu2mean1");
  fitresPFu2mean1 = grPFu2mean1->Fit("fcnPFu2mean1","QMRN0FBSE");
  sprintf(chi2ndf,"#chi^{2}/ndf = %.2f",(fcnPFu2mean1->GetChisquare())/(fcnPFu2mean1->GetNDF()));  
//  errBand->SetPoint(0,0.002*(xval[nbins-1]),fcnPFu2mean->Eval(0.002*(xval[nbins-1])));
//  errBand->SetPointError(0,0,dMean(fcnPFu2mean,0.002*(xval[nbins-1]),fitresPFu2mean));
//  for(Int_t i=1; i<=nbins; i++) {
//    errBand->SetPoint(i,xval[i-1],fcnPFu2mean->Eval(xval[i-1]));
...
//  errBand->SetPoint(nbins+1,1.2*(xval[nbins-1]),fcnPFu2mean->Eval(1.2*(xval[nbins-1])));
//  errBand->SetPointError(nbins+1,0,dMean(fcnPFu2mean,1.2*(xval[nbins-1]),fitresPFu2mean));    
  CPlot plotPFu2mean1("pfu2mean1","","p_{T}(ll) [GeV/c]","#mu_{1}(u_{2}) [GeV]");
  plotPFu2mean1.AddGraph(grPFu2mean1,"");
//  plotPFu2mean.AddGraph(errBand,"3",kAzure-9,kFullDotSmall);
  plotPFu2mean1.AddGraph(grPFu2mean1,"",kBlack,kOpenCircle);
  plotPFu2mean1.AddFcn(fcnPFu2mean1,kRed);
  plotPFu2mean1.AddTextBox(chi2ndf,0.21,0.87,0.41,0.82,0,kBlack,-1);
  sprintf(fitparam,"p_{0} = %.3f #pm %.3f",fcnPFu2mean1->GetParameter(0),fcnPFu2mean1->GetParError(0)); plotPFu2mean1.AddTextBox(fitparam,0.21,0.80,0.51,0.75,0,kBlack,-1);
  sprintf(fitparam,"p_{1} = %.3f #pm %.3f",fcnPFu2mean1->GetParameter(1),fcnPFu2mean1->GetParError(1)); plotPFu2mean1.AddTextBox(fitparam,0.21,0.75,0.51,0.70,0,kBlack,-1);
  plotPFu2mean1.Draw(c,kTRUE,"png");
...
//  errBand->SetPoint(nbins+1,1.2*(xval[nbins-1]),fcnPFu2mean->Eval(1.2*(xval[nbins-1])));
//  errBand->SetPointError(nbins+1,0,dMean(fcnPFu2mean,1.2*(xval[nbins-1]),fitresPFu2mean));    
  CPlot plotPFu2mean2("pfu2mean2","","p_{T}(ll) [GeV/c]","#mu_{2}(u_{2}) [GeV]");
...
//    else           plotPFu2sigma0.SetYRange(6,20);
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
    ...
    for(Int_t ientry=0; ientry<intree->GetEntries(); ientry++) {
      ...
      Double_t zpfu1 = (u1 - pfu1Mean1[ipt])/(pfu1Sigma0[ipt]);
      Double_t zpfu2 = (u2 - pfu2Mean1[ipt])/(pfu2Sigma0[ipt]);
    
      for(Int_t ibin=0; ibin<ncorrbins; ibin++) {
        if(dilep->Pt() > corrbins[ibin] && dilep->Pt() <= corrbins[ibin+1]) {
          hPFu1u2v[ibin]->Fill(fabs(zpfu1),fabs(zpfu2)/*,scale1fb*puWeight*10000*/);	
        }
      }
      if(dilep->Pt() > corrbins[ncorrbins]) {
        hPFu1u2v[ncorrbins-1]->Fill(fabs(zpfu1),fabs(zpfu2)/*,scale1fb*puWeight*10000*/);    
      }
    }
  }
  delete infile;
  ...
  if(grPFu1mean1)   grPFu1mean1->Write();
  ...
  if(fcnPFu1mean1)  fcnPFu1mean1->Write();
  ...
  fitresPFu1mean1->SetName("fitresPFu1mean1");     fitresPFu1mean1->Write();
  fitresPFu1mean2->SetName("fitresPFu1mean2");     fitresPFu1mean2->Write();
  ...
  if(grPFu2mean1)   grPFu2mean1->Write();
  ...
  if(fcnPFu2mean1)   fcnPFu2mean1->Write();
  ...
  fitresPFu2mean1->SetName("fitresPFu2mean1");     fitresPFu2mean1->Write();
  fitresPFu2mean2->SetName("fitresPFu2mean2");     fitresPFu2mean2->Write();
              ...
              const Int_t pfu1model, const Int_t pfu2model)
{
  ...
  if(pfu1model==1) {
    ...
    htmlfile << "<tr>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1mean.png\"><img src=\"plots/pfu1mean.png\" alt=\"plots/pfu1mean.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1mean1.png\"><img src=\"plots/pfu1mean1.png\" alt=\"plots/pfu1mean1.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1mean2.png\"><img src=\"plots/pfu1mean2.png\" alt=\"plots/pfu1mean2.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1sigma0.png\"><img src=\"plots/pfu1sigma0.png\" alt=\"plots/pfu1sigma0.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1sigma1.png\"><img src=\"plots/pfu1sigma1.png\" alt=\"plots/pfu1sigma1.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1sigma2.png\"><img src=\"plots/pfu1sigma2.png\" alt=\"plots/pfu1sigma2.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu1frac2.png\"><img src=\"plots/pfu1frac2.png\" alt=\"plots/pfu1frac2.png\" width=\"100%\"></a></td>" << endl;    
    htmlfile << "</tr>" << endl;
  ...
  if(pfu2model==1) {
    ...
    htmlfile << "<tr>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2mean.png\"><img src=\"plots/pfu2mean.png\" alt=\"plots/pfu2mean.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2mean1.png\"><img src=\"plots/pfu2mean1.png\" alt=\"plots/pfu2mean1.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2mean2.png\"><img src=\"plots/pfu2mean2.png\" alt=\"plots/pfu2mean2.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2sigma0.png\"><img src=\"plots/pfu2sigma0.png\" alt=\"plots/pfu2sigma0.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2sigma1.png\"><img src=\"plots/pfu2sigma1.png\" alt=\"plots/pfu2sigma1.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2sigma2.png\"><img src=\"plots/pfu2sigma2.png\" alt=\"plots/pfu2sigma2.png\" width=\"100%\"></a></td>" << endl;
    htmlfile << "<td width=\"12%\"><a target=\"_blank\" href=\"plots/pfu2frac2.png\"><img src=\"plots/pfu2frac2.png\" alt=\"plots/pfu2frac2.png\" width=\"100%\"></a></td>" << endl;    
    htmlfile << "</tr>" << endl;
...
void performFit(const vector<TH1D*> hv, const vector<TH1D*> hbkgv, const Double_t *ptbins, const Int_t nbins,
  ...
  for(Int_t ibin=0; ibin<nbins; ibin++) {
    ...
    RooGaussian gauss3("gauss3","gauss3",u,mean1,sigma3);
    ...
    //
    char formula[100];
    char formulaMean[100];
    RooArgList params;
    RooArgList paramsMean;
    if(model==1) {
      sprintf(formula,"sigma1");
    
    } else if(model==2) {
      sprintf(formula,"(1.-frac2)*sigma1 + frac2*sigma2");
      sprintf(formulaMean,"(1.-frac2)*mean1 + frac2*mean2");
      params.add(frac2);
      params.add(sigma1);
      params.add(sigma2);
      paramsMean.add(frac2);
      paramsMean.add(mean1);
      paramsMean.add(mean2);
    ...
    RooFormulaVar mean("mean",formulaMean,paramsMean);
    ...
    if(sigma1.getVal() > sigma2.getVal()) {
      ...
      Double_t wide_mean = sigma1.getVal();
      Double_t thin_mean = sigma2.getVal();
      sigma1.setVal(thin);
      sigma2.setVal(wide);
      mean1.setVal(thin_mean);
      mean2.setVal(wide_mean);
    ...
    if(model>=2) {
      ...
      meanErrArr[ibin]   = mean.getPropagatedError(*fitResult);
...
//     sprintf(mean1text,"#mu_1 = %.1f #pm %.1f",mean1Arr[ibin],mean1ErrArr[ibin]);
//     sprintf(mean2text,"#mu_2 = %.1f #pm %.1f",mean2Arr[ibin],mean2ErrArr[ibin]);
    ...
    if(model==1)      plot.AddTextBox(0.70,0.90,0.95,0.80,0,kBlack,-1,2,meantext,mean1text,mean2text,sig1text);
    else if(model==2) plot.AddTextBox(0.70,0.90,0.95,0.70,0,kBlack,-1,4,meantext,mean1text,mean2text,sig0text,sig1text,sig2text);
    else if(model==3) plot.AddTextBox(0.70,0.90,0.95,0.65,0,kBlack,-1,5,meantext,mean1text,mean2text,sig0text,sig1text,sig2text,sig3text);
