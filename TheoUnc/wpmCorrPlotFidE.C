...
void wpmCorrPlotFidE() {
  ...
  Double_t wm_xs_meas = 3.92;

  //statistical
  Double_t wp_stat = 0.05;
  Double_t wm_stat = 0.04;

  //lepton reco+id uncertainties
  Double_t wp_lep = 3.2*0.01; 
  Double_t wm_lep = 3.2*0.01;

  //background subtraction/modelling
  Double_t wp_bkg = 0.9*0.01;
  Double_t wm_bkg = 1.9*0.01;

  //lumi
  Double_t wp_lumi = 0.60;
  Double_t wm_lumi = 0.47;

  CorrPlot plot("cplot","","#sigma^{acc}_{W}xBR(W^{-}#rightarrow e#nu) [nb]","#sigma^{acc}_{W}xBR(W^{+}#rightarrow e#nu) [nb]",3.4,4.5,4.3,5.7);
  ...
  plot.Draw(c1, "wpm_fid_ele.png",42);
