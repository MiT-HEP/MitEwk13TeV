...
void wpmCorrPlotFidE() {
  ...
  Double_t wp_xs_meas = 4901.02;
  Double_t wm_xs_meas = 3828.44;

  //statistical
  Double_t wp_stat = 39.1497;
  Double_t wm_stat = 37.0537;

  //lepton reco+id uncertainties
  Double_t wp_lep = 1.616199*0.01; 
  Double_t wm_lep = 1.60228*0.01;

  //background subtraction/modelling
  Double_t wp_bkg = 1.372443*0.01;
  Double_t wm_bkg = 1.372443*0.01;

  //lumi
  Double_t wp_lumi = 235.328;
  Double_t wm_lumi = 183.945;

  CorrPlot plot("cplot","","#sigma^{fid}_{W^{-}}xBR(W^{-}#rightarrow e#nu) [pb]","#sigma^{fid}_{W^{+}}xBR(W^{+}#rightarrow e#nu) [pb]",3500,4200,4300,5400);
  ...
  toVec(folder+"/wme_nnpdf30.txt",
	nnpdf23_minus);

  std::vector<Double_t> nnpdf23_plus;
  toVec(folder+"/wpe_nnpdf30.txt",
	nnpdf23_plus);
  ...
  toVec(folder+"/wme_mmht2014.txt",
	mstw2008_minus);

  std::vector<Double_t> mstw2008_plus;
  toVec(folder+"/wpe_mmht2014.txt",
	mstw2008_plus);
  ...
  toVec(folder+"/wme_abm.txt",
	abm_minus);
  
  std::vector<Double_t> abm_plus;
  toVec(folder+"/wpe_abm.txt",
	abm_plus);
  ...
  toVec(folder+"/wme_hera.txt",
	hera_minus);
  
  std::vector<Double_t> hera_plus;
  toVec(folder+"/wpe_hera.txt",
	hera_plus);
  ...
  plot.Draw(c1, "wpm_fid_ele.png",43);
